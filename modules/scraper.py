...
SORT_OPTIONS = {
    "newest": (
        "Newest", "╫Ф╫Ч╫У╫й╫Х╫к ╫С╫Щ╫Х╫к╫и", "р╣Гр╕лр╕бр╣Ир╕Чр╕╡р╣Ир╕кр╕╕р╕Ф", "цЬАцЦ░", "M├бs recientes", "цЬАш┐С",
        "Mais recentes", "Neueste", "Plus r├йcent", "Pi├╣ recenti", "Nyeste",
        "╨Э╨╛╨▓╤Л╨╡", "Nieuwste", "╪м╪п┘К╪п", "Nyeste", "Uusimmat", "Najnowsze",
        "Senaste", "Terbaru", "Yak─▒n zamanl─▒", "Mс╗Ыi nhс║еt", "рдирд╡реАрдирддрдо"
    ),
    "highest": (
        "Highest rating", "╫Ф╫У╫Щ╫и╫Х╫Т ╫Ф╫Т╫С╫Х╫Ф ╫С╫Щ╫Х╫к╫и", "р╕Др╕░р╣Бр╕Щр╕Щр╕кр╕╣р╕Зр╕кр╕╕р╕Ф", "цЬАщлШшйХф╛б",
        "Calificaci├│n m├бs alta", "цЬАщлШшпДхИЖ", "Melhor avalia├з├гo", "H├╢chste Bewertung",
        "Note la plus ├йlev├йe", "Valutazione pi├╣ alta", "H├╕yeste vurdering",
        "╨Э╨░╨╕╨▓╤Л╤Б╤И╨╕╨╣ ╤А╨╡╨╣╤В╨╕╨╜╨│", "Hoogste waardering", "╪г╪╣┘Д┘Й ╪к┘В┘К┘К┘Е", "H├╕jeste vurdering",
        "Korkein arvostelu", "Najwy┼╝sza ocena", "H├╢gsta betyg", "Peringkat tertinggi",
        "En y├╝ksek puan", "─Р├бnh gi├б cao nhс║еt", "рдЙрдЪреНрдЪрддрдо рд░реЗрдЯрд┐рдВрдЧ", "Top rating"
    ),
    "lowest": (
        "Lowest rating", "╫Ф╫У╫Щ╫и╫Х╫Т ╫Ф╫а╫Ю╫Х╫Ъ ╫С╫Щ╫Х╫к╫и", "р╕Др╕░р╣Бр╕Щр╕Щр╕Хр╣Ир╕│р╕кр╕╕р╕Ф", "цЬАф╜ОшйХф╛б",
        "Calificaci├│n m├бs baja", "цЬАф╜ОшпДхИЖ", "Pior avalia├з├гo", "Niedrigste Bewertung",
        "Note la plus basse", "Valutazione pi├╣ bassa", "Laveste vurdering",
        "╨Э╨░╨╕╨╝╨╡╨╜╤М╤И╨╕╨╣ ╤А╨╡╨╣╤В╨╕╨╜╨│", "Laagste waardering", "╪г┘В┘Д ╪к┘В┘К┘К┘Е", "Laveste vurdering",
        "Alhaisin arvostelu", "Najni┼╝sza ocena", "L├дgsta betyg", "Peringkat terendah",
        "En d├╝┼Я├╝k puan", "─Р├бnh gi├б thс║еp nhс║еt", "рдирд┐рдореНрдирддрдо рд░реЗрдЯрд┐рдВрдЧ", "Worst rating"
    ),
    "relevance": (
        "Most relevant", "╫и╫Ь╫Х╫Х╫а╫Ш╫Щ╫Х╫к ╫С╫Щ╫Х╫к╫и", "р╣Ар╕Бр╕╡р╣Ир╕вр╕зр╕Вр╣Йр╕нр╕Зр╕бр╕▓р╕Бр╕Чр╕╡р╣Ир╕кр╕╕р╕Ф", "щЦвщАгцАз",
        "M├бs relevantes", "цЬАчЫ╕хЕ│", "Mais relevantes", "Relevanteste",
        "Plus pertinents", "Pi├╣ pertinenti", "Mest relevante",
        "╨Э╨░╨╕╨▒╨╛╨╗╨╡╨╡ ╤А╨╡╨╗╨╡╨▓╨░╨╜╤В╨╜╤Л╨╡", "Meest relevant", "╪з┘Д╪г┘Г╪л╪▒ ╪╡┘Д╪й", "Mest relevante",
        "Olennaisimmat", "Najbardziej trafne", "Mest relevanta", "Paling relevan",
        "En alakal─▒", "Li├кn quan nhс║еt", "рд╕рдмрд╕реЗ рдкреНрд░рд╛рд╕рдВрдЧрд┐рдХ", "Relevance"
    )
}
...
REVIEW_WORDS = {
    ...
    # Hebrew
    "╫С╫Щ╫з╫Х╫и╫Х╫к", "╫С╫Щ╫з╫Х╫и╫к", "╫С╫Щ╫з╫Х╫и╫Х╫к ╫в╫Ь", "╫У╫Щ╫и╫Х╫Т╫Щ╫Э", "╫У╫Щ╫и╫Х╫Т",

    # Thai
    "р╕гр╕╡р╕зр╕┤р╕з", "р╕Ър╕Чр╕зр╕┤р╕Ир╕▓р╕гр╕Ур╣М", "р╕Др╕░р╣Бр╕Щр╕Щ", "р╕Др╕зр╕▓р╕бр╕Др╕┤р╕Фр╣Ар╕лр╣Зр╕Щ",

    # Spanish
    "rese├▒as", "opiniones", "valoraciones", "cr├нticas", "calificaciones",

    # French
    "avis", "commentaires", "├йvaluations", "critiques", "notes",
    ...
    # Portuguese
    "avalia├з├╡es", "coment├бrios", "opini├╡es", "an├бlises", "cr├нticas",

    # Russian
    "╨╛╤В╨╖╤Л╨▓╤Л", "╤А╨╡╤Ж╨╡╨╜╨╖╨╕╨╕", "╨╛╨▒╨╖╨╛╤А╤Л", "╨╛╤Ж╨╡╨╜╨║╨╕", "╨║╨╛╨╝╨╝╨╡╨╜╤В╨░╤А╨╕╨╕",

    # Japanese
    "уГмуГУуГеуГ╝", "хПгуВ│уГЯ", "шйХф╛б", "цЙ╣шйХ", "цДЯцГ│",

    # Korean
    "ыжмы╖░", "эПЙъ░А", "эЫДъ╕░", "ыМУъ╕А", "ьЭШъ▓м",

    # Chinese (Simplified and Traditional)
    "шпДшо║", "шйХшлЦ", "чВ╣шпД", "щ╗ЮшйХ", "шпДф╗╖", "шйХхГ╣", "цДПшзБ", "цДПшжЛ", "хЫЮщб╛", "хЫЮщбз",

    # Arabic
    "┘Е╪▒╪з╪м╪╣╪з╪к", "╪к┘В┘К┘К┘Е╪з╪к", "╪в╪▒╪з╪б", "╪к╪╣┘Д┘К┘В╪з╪к", "┘Ж┘В╪п",

    # Hindi
    "рд╕рдореАрдХреНрд╖рд╛", "рд░рд┐рд╡реНрдпреВ", "рд░рд╛рдп", "рдореВрд▓реНрдпрд╛рдВрдХрди", "рдкреНрд░рддрд┐рдХреНрд░рд┐рдпрд╛",

    # Turkish
    "yorumlar", "de─Яerlendirmeler", "incelemeler", "g├╢r├╝┼Яler", "puanlar",
    ...
    # Vietnamese
    "─С├бnh gi├б", "nhс║нn x├йt", "b├мnh luс║нn", "phс║гn hс╗Уi", "b├аi ─С├бnh gi├б",

    # Indonesian
    "ulasan", "tinjauan", "komentar", "penilaian", "pendapat",

    # Swedish
    "recensioner", "betyg", "omd├╢men", "├еsikter", "kommentarer",

    # Norwegian
    "anmeldelser", "vurderinger", "omtaler", "meninger", "tilbakemeldinger",

    # Danish
    "anmeldelser", "bed├╕mmelser", "vurderinger", "meninger", "kommentarer",

    # Finnish
    "arvostelut", "arviot", "kommentit", "mielipiteet", "palautteet",

    # Greek
    "╬║╧Б╬╣╧Д╬╣╬║╬н╧В", "╬▒╬╛╬╣╬┐╬╗╬┐╬│╬о╧Г╬╡╬╣╧В", "╧Г╧З╧М╬╗╬╣╬▒", "╬▒╧А╧М╧И╬╡╬╣╧В", "╬▓╬▒╬╕╬╝╬┐╬╗╬┐╬│╬п╬╡╧В",

    # Czech
    "recenze", "hodnocen├н", "n├бzory", "koment├б┼Щe", "posudky",

    # Romanian
    "recenzii", "evalu─Гri", "opinii", "comentarii", "note",

    # Hungarian
    "v├йlem├йnyek", "├йrt├йkel├йsek", "kritik├бk", "hozz├бsz├│l├бsok", "megjegyz├йsek",
...
class GoogleReviewsScraper:
    ...
    def setup_driver(self, headless: bool):
            ...
            for i in ['', '_2', '_3', '_4', '_5']:
                ...
                if proxy_url:
                    ...
                    if ':10000' in proxy_url:
                        ...
                        log.info(f"ЁЯФД Using random port: {random_port} for IP rotation")
                    
                    proxy_urls.append(proxy_url)
                    log.info(f"тЬЕ Proxy {len(proxy_urls)} configured: {proxy_url.split('@')[-1] if '@' in proxy_url else proxy_url}")
        ...
        # Create chromium args list for proxy if configured
        chromium_args = []
        if proxy_url:
            ...
            if chrome_binary and os.path.exists(chrome_binary):
                try:
                    driver = Driver(
                        ...
                        chromium_arg=','.join(chromium_args) if chromium_args else None,
    ...
    def set_sort(self, driver: Chrome, method: str):
        ...
        try:
            ...
            sort_button_selectors = [
                ...
                'button[aria-label*="╫б╫У╫и" i]',  # Hebrew
                'button[aria-label*="р╣Ар╕гр╕╡р╕вр╕З" i]',  # Thai
                'button[aria-label*="цОТх║П" i]',  # Chinese
                ...
                try:
                    ...
                    for element in elements:
                        try:
                            ...
                            negative_keywords = ["back", "next", "previous", "close", "cancel", "╫Ч╫Ц╫и╫Ф", "╫б╫Т╫Х╫и", "р╕Ыр╕┤р╕Ф"]
                            if any(keyword in button_text.lower() or keyword in button_aria.lower()
                                   for keyword in negative_keywords):
                                continue

                            # Positive detection for sort buttons
                            sort_keywords = ["sort", "Sort", "SORT", "╫б╫Щ╫У╫Х╫и", "р╣Ар╕гр╕╡р╕вр╕З", "цОТх║П", "trier", "ordenar", "sortieren"]
            ...
            if not sort_button:
                xpath_terms = ["sort", "Sort", "╫б╫У╫и", "╫б╫Щ╫У╫Х╫и", "р╣Ар╕гр╕╡р╕вр╕З", "цОТх║П", "Trier", "Ordenar", "Sortieren"]
    ...
    def scrape(self):
        ...
        try:
                ...
                try:
                    ...
                    for card in fresh_cards:
                        try:
                            ...
                            log.warning("тЪая╕П parse error тАУ storing stub\n%s",
            ...
            log.info("тЬЕ Finished тАУ total unique reviews: %s", len(docs))
...
#     "newest": ("Newest", "╫Ф╫Ч╫У╫й╫Х╫к ╫С╫Щ╫Х╫к╫и", "р╣Гр╕лр╕бр╣Ир╕Чр╕╡р╣Ир╕кр╕╕р╕Ф"),
#     "highest": ("Highest rating", "╫Ф╫У╫Щ╫и╫Х╫Т ╫Ф╫Т╫С╫Х╫Ф ╫С╫Щ╫Х╫к╫и", "р╕Др╕░р╣Бр╕Щр╕Щр╕кр╕╣р╕Зр╕кр╕╕р╕Ф"),
#     "lowest": ("Lowest rating", "╫Ф╫У╫Щ╫и╫Х╫Т ╫Ф╫а╫Ю╫Х╫Ъ ╫С╫Щ╫Х╫к╫и", "р╕Др╕░р╣Бр╕Щр╕Щр╕Хр╣Ир╕│р╕кр╕╕р╕Ф"),
#     "relevance": ("Most relevant", "╫и╫Ь╫Х╫Х╫а╫Ш╫Щ╫Х╫к ╫С╫Щ╫Х╫к╫и", "р╣Ар╕Бр╕╡р╣Ир╕вр╕зр╕Вр╣Йр╕нр╕Зр╕бр╕▓р╕Бр╕Чр╕╡р╣Ир╕кр╕╕р╕Ф"),
# }
#
# REVIEW_WORDS = {"reviews", "review", "╫С╫Щ╫з╫Х╫и╫Х╫к", "р╕гр╕╡р╕зр╕┤р╕з", "avis", "rese├▒as",
#                 "recensioni", "bewertungen", "хПгуВ│уГЯ", "уГмуГУуГеуГ╝",
#                 "ыжмы╖░", "шйХшлЦ", "шпДшо║", "╤А╨╡╤Ж╨╡╨╜╨╖╨╕╨╕", "╫С╫Щ╫з╫Х╫и╫к"}
#
#
# class GoogleReviewsScraper:
#     """Main scraper class for Google Maps reviews"""
...
# #                 return
# #         log.warning("тЪая╕П  sort option %s not found тАУ keeping default", method)
# #
# #     def scrape(self):
# #         """Main scraper method"""
# #         start_time = time.time()
